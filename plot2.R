## This script gives an answer are total emissions from PM2.5 have decreased in the city of Baltimore 
...
balsum <- aggregate(neibal$emissions, by = list(neibal$year), FUN = sum)

## Define more user friendly coulmn names for summarized data set
names(balsum) <- c("year", "total")

## Initialize graphics device
png(filename = "plot2.png", width=480, height=480)

## Create a bar plot showing total emissions per year for city of Baltimore
barplot(height = balsum[, "total"], names.arg = balsum[,"year"], las = 2, cex.axis = 0.7, 
        cex.names = 0.7, main = "PM emissions in Baltimore over years", ylab = "Total PM emission")
